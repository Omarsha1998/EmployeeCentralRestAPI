    ...
    try {
      ...
      if (!util.empty(req.query.semester)) {
        if (Number(req.query.semester) >= "20241") {
          conditions = `and currentChoice = 1 and appStatus = 1 and sem = ? and active = 1`;
        } else {
      ...
      if (!util.empty(req.query.college)) {
        conditions =
          "and sem = ? and currentChoice = 1 and appStatus = 1 and college = ? and active = 1";
      ...
      if (!util.empty(req.query.forAcceptance)) {
        conditions = `and (forAcceptance = ? or accepted = ?) and sem = '${req.query.semester}' and college not in ('G') and currentChoice = 1 and appStatus = 1  and active = 1`;
        args = [1, 1];
      }

      const studentApplications =
        await applicationsModel.selectStudentApplications(
          conditions,
          args,
          {
            order: "fullName",
  ...
  const intervalId = await setInterval(async () => {
    const returnValue = await sqlHelper.transact(async (txn) => {
      try {
        ...
        if (!util.empty(req.query.college)) {
          conditions = "and sem = ? and currentChoice = 1 and college = ?";
          args = [req.query.semester, req.query.college];
        }

        if (!util.empty(req.query.withEnrollment)) {
          // conditions =  "and [FOR INTERVIEW] = 1 and sem = ? and currentChoice = 1 and college = ?";
          if (req.query.college === "G") {
            conditions = " and sem = ? and currentChoice = 1 and college = ?";
            args = [req.query.semester.replace("1", "3"), req.query.college];
          } else {
        ...
        if (!util.empty(req.query.forAcceptance)) {
          conditions = `and (forAcceptance = ? or accepted = ?) and sem like '%${req.query.semester}%' and college not in ('M', 'G')`;
          args = [1, 1];
        }

        const studentApps = await applicationsModel.selectStudentApplications(
          conditions,
          args,
          {
            order: "fullName",
  ...
  const returnValue = await sqlHelper.transact(async (txn) => {
    try {
      ...
      for (const file in req.files) {
        const filePayload = {
          [type]: req.files[file].data,
          ...fileType,
        };
        payload = filePayload;
      }
      return await applicantsModel.updateApplicants(
        payload,
...
const getFile = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    // let fileTypeVal = `${req.query.fileType ? `${req.query.type}type` : ``}`;
    // let fileType = `${req.query.fileType ? `, ${fileTypeVal}` : ""} `;
    // let column = `${req.query.type} ${fileType}`;
    ...
    try {
      ...
      // return true
      // if (!util.empty(getFile[0][req.query.type])) {
      //   return {
      //     value: Buffer.from(getFile[0][req.query.type]),
      //     fileTypeValue:
      //       getFile[0][fileTypeVal] === undefined
      //         ? ""
      //         : getFile[0][fileTypeVal],
    ...
    try {
      ...
      if (applicationInfo.application_status === "FOR ACCEPTANCE") {
        ...
        const level2Interviewer = await interviewerModel.selectInterviewers(
          "and interviewerLevel = 2 and college = ?",
          [admissionDetails.course],
          {
            order: "fullName asc",
            top: "",
          },
