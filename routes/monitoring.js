  ...
  void (async function () {
    try {
      ...
      if (result.recordset.length > 0) {
        for (let dtr of result.recordset) {
  ...
  void (async function () {
    try {
      ...
      if (otCalTypeQuery.recordset.length > 0) {
        ...
        for (let otCal of otCalRecordset) {
          otCal.hours =
      ...
      if (finalOTRecordset.length > 0) {
        for (let otData of finalOTRecordset) {
          otData.date = await util.formatDate( { date: otData.date.toISOString(), straightDate: true })
        }
      }
      ...
      if (late.recordset.length > 0) {
        for (let lateData of late.recordset) {
          lateData.transDate = await util.formatDate( { date: lateData.transDate.toISOString(), straightDate: true })
        }
      }
      ...
      computed.habitualTardiness = dtr.recordset.map((i) => {
        ...
        var timeIn = new Date(`${formattedTransDate} ${i.IN}`);
        var scheduleFrom = new Date(`${formattedTransDate} ${i.schedFrom}`);
        let earlyIn = 0;
        if (timeIn < scheduleFrom) {
          var diff = scheduleFrom.getTime() - timeIn.getTime();
          var msec = diff;
      ...
      for (var computation of computed.habitualTardiness) {
