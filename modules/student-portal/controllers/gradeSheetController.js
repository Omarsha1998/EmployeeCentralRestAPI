...
const suggestedSubject = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    try {
      // console.log("Student No", req.query.studentno);
      // const verifyWhere = `SN = '${req.query.studentno}'`;
      // const verify = await grades.checkIfHasFailedSubject(verifyWhere, txn);
      // console.log("VERIFIED", verify);

      const semester = `${req.query.semester}`;
      const removeSY = semester.replace("SY ", "");
      const sqlWhere = `SchoolYear = '${removeSY}'`;
...
const getSubjectCode = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    try {
      ...
      const semester = `${req.query.semester}%`;
      const removeSY = semester.replace("SY ", "");
      const sqlWhere = `Description LIKE '${escapedSearchTerm}' AND SchoolYear LIKE '${removeSY}'`;
...
const creditUnits = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    try {
      // console.log("HERE CREDITS: ", req.query.subjectcode)
...
const addCourses = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    const sqlWhere = `SchoolYear = '${course.schoolYear}' AND SubjectCode = '${course.subjectCode}' AND YearLevel = '${course.yearLevel}'`;
    const verified = await grades.verifiedCourseCode(sqlWhere, txn);

    if (verified.length === 1) {
      return res.json({
        message: "Subject codes already exist in this specific school year.",
      });
    }
