...
const getRRNumber = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    let receivingReportNox = req.query.receivingReportNo;
    // console.log("receivingReportNox",receivingReportNox)
    let sqlWhere = "";
    // const userDepartmentCode = util.currentUserToken(req).deptCode
    let args = [];
    sqlWhere = ` and receivingReportNo = ? `;
    args = [receivingReportNox];
    let options = {
      top: "",
      order: "itAssetCode desc",
    };
    return await assets.selectAssets(
      sqlWhere,
      args,
...
// const getLastAraNo = async function (req, res) {
//   const returnValue = await sqlHelper.transact(async (txn) => {
    

//     let sqlWhere = "";
//     let args = [];
//     sqlWhere = `and araForm LIKE 'ARA-%'`;
//     args = [];
//     let options = {
//       top: "1",
//       order: "araForm desc",
...
const findAraNoDirect = async function (txn) {
  let  sqlWhere = `and araForm LIKE 'ARA-%'`;
  let args = [];
  let options = {
    top: "1",
      order: "araForm desc",
  ...
  const returnValue = await allotmentHistory.selectCondemHistory(sqlWhere, args, options, txn);
...
const postPartsTransfer = async function (req, res) {
  ...
  const returnValue = await sqlHelper.transact(async (txn) => {
    const { assetsInfo,transferToDeptCode,remarks,assetCodeResult,tfNumber} = req.body;
    ...
    let incrementedPart;
     ...
     const formattedDate = formatDate(util.currentDateTime());
       ...
       let [pref, deyt, numericPart] = tfNumber.split('-');
       ...
       if (formattedDate !== deyt) { 
           ...
           incrementedPart = (toInts + 1).toString().padStart(numericPart.length, '0');
           nextAraNumber = `TF-${formattedDate}-${incrementedPart}`;
       }
...
const postAssetsTransfer = async function (req, res) {
  ...
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    // let generatedTransferFormCode = "";
    const { assetsInfo,transferToDeptCode,remarks,partsInclude,tfNumber} = req.body;
...
try {
   ...
   const returnValue = await sqlHelper.transact(async (txn) => {
    let araTop1 = await findAraNoDirect(txn); 
    let araME;
    if (araTop1.length > 0) {
      // console.log("araTop1", araTop1[0].araForm);
 ...
 let incrementedPart;
  ...
  const formattedDate = formatDate(util.currentDateTime());
    ...
    let [pref, deyt, numericPart] = araME.split('-');
...
const postjsonData = async function (req, res) {
  ...
  const returnValue = await sqlHelper.transact(async (txn) => {
    try {
      ...
      assetStatus = await sqlHelper.transact(async (txn) => {
        let allAssets1 = await findAssetCodeDirect(txn);  
        for (let jsonData of nonEmptyRows) {
          jsonData = trimObjectStrings(jsonData); 
          let generatedCodeUniqueAsset = await sqlHelper.generateUniqueCode(
            "UERMINV..Assets",
            `AUPLOAD`,
            6,
            txn
         ...
         if (equipmentType === 'Whole' && jsonData['TYPE'] === 'Whole'){
            ...
            let checkCode = jsonData['ASSET CODE OF MAIN ITEM'] || jsonData[' ASSET CODE']
 let tempoSave;
 let finalApprovalStatus='Approved';
      let assetExists = allAssets1.find(asset => asset.oldAssetCode === checkCode);
      if(assetExists){
         ...
         finalApprovalStatus='Approved(with whole)'
            ...
            let jsonPayload = {
              ...
              BrandName: jsonData['BRAND'],
              ItemCode: jsonData['ITEM CODE'],
              AssetCode: jsonData['ASSET CODE OF MAIN ITEM'] || jsonData[' ASSET CODE'],
              Code: generatedCode,
              GenericName: jsonData['GENERIC NAME'],
              CreatedBy: uploadedBy,
              UpdatedBy: uploadedBy,
              ...
              Location: jsonData['LOCATION'],
              Capitalized: jsonData['CAPITALIZED'],
              CondemnReStatus: jsonData['FINAL APPROVAL'] ? finalApprovalStatus:'N/A' ,
              AraForm: jsonData['ARA FORM NO.'],
...
const postRegisterAssetCE = async function (req, res) {
  if (util.empty(req.body))
    return res.status(400).json({ error: "`body` is required." });
  const { assetsInfo, assetCodeResult, itAssetCodeResult, entries,rrcounts,testRrCountsMax,rrUpdate } = req.body;
  try {    
    const formatDate = (dateStr) => {
      const date = new Date(dateStr);
      const year = date.getFullYear();
    ...
    const formattedDate = formatDate(assetsInfo.dateReceived);
    const testRrCountss =rrcounts.length

let numerator; 
if (rrcounts.length >0){
  let countsMa = testRrCountsMax
  countsMa = String(countsMa); 
  let [dept, keyw,dtcrt, fract] = countsMa.split('-');
  let num= fract.split('/');
     numerator = parseInt(num[0]) //10

}
else{
  const defaultCount = 0;
  numerator = defaultCount
}

    const totalSetss = parseInt(entries) + parseInt(testRrCountss)
  
    const [prefix, category,defaultStartPart, lastPart] = assetCodeResult.split('-');
 ...
 let numee2;
 let numeratorParse;
//  console.log("rrcounts.length",rrcounts.length)
        if (rrcounts.length >0){ //17
          numeratorParse =  parseInt(numerator) //10
          // console.log("numeratorParse",numeratorParse)
         const plusOne = `${numeratorParse + 1}`;
         const addedOneResult = parseInt(plusOne)
  numee2 = `${addedOneResult + i}`; //11
  // console.log("numee2",numee2)
        }
        else{
          numee2 = `${numerator + i}`;
          // console.log("numee2",numee2)
        ...
        const numee = parseInt(numee2)//11
        // console.log("numee",numee)
        let newAssetsCode="";
        let itAssetCodeByBatch = `${itAssetCodeResult.toUpperCase()}-${formattedDate}-${numee + 1}` //${totalSetss}
        // console.log("itAssetCodeByBatch",itAssetCodeByBatch)
        if (assetCodeResult !=="") {
          const incrementedPart = (lastPartAsInt + i).toString().padStart(3, '0');
           newAssetsCode = `${prefix.toUpperCase()}-${category.toUpperCase()}-${defaultStartPart}-${incrementedPart}`;
        }
        let generatedCode = "";
        // let assetStatus = "";
        const catCode = "CE";
      ...
      // If all assets inserted successfully, return success
      if (rrcounts.length >=1){
      let updateAssets =   await assets.updateAssets(
        {
          totalSets:totalSetss
          
        },
        { receivingReportNo: rrUpdate },
        txn
      );
...
// const getRRCount = async function (req, res) {
//   const returnValue = await sqlHelper.transact(async (txn) => {
//     if (util.empty(req.query.araForm))
//       return res.status(400).json({ error: "`Ara Form No` is required." });
...
const postRegisterAsset = async function (req, res) {
  if (util.empty(req.body))
    ...
    try { 
      ...
      // console.log(" defaultStartPart",defaultStartPart )
      // console.log(" lastPart",lastPart )
      // console.log("oldAssetCode",oldAssetCode)
      // const parts = oldAssetCode.split('-');
...
let lastPartAsInt = parseInt(lastPart, 10)
  ...
  const combi = `${prefix.toUpperCase()}-${category.toUpperCase()}`
          const newAssetsCode = `${combi}-${defaultStartPart}-${incrementedPart}`;
