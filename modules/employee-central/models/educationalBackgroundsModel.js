  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (await helperMethods.isFolderExist(path) === true) {
      if (await helperMethods.isFolderEmpty(path) === false) {
        const url = `${helperMethods.getURL()  }/${process.env.EC_EJS_VIEWS_FOLDER}/uploads/get-current-tor-or-diploma?token=${  token}`;
...
const parameters = [{ name: "Description", dataType: "VarChar", value: degreeName }];
...
async function isCourseExist(courseName){
  const query = `SELECT 
                 ...
                 WHERE [Description] = @Description`;
const parameters = [{ name: "Description", dataType: "VarChar", value: courseName }];
const response = await helperMethods.executeQuery(query, parameters);
return (response.recordset[0].total !== 0 && response.recordset[0].total > 0) ? true : false;
}
...
async function isMajorExist(majorName){
  const query = `SELECT 
                 ...
                 WHERE [Description] = @Description`;
const parameters = [{ name: "Description", dataType: "VarChar", value: majorName }];
const response = await helperMethods.executeQuery(query, parameters);
return (response.recordset[0].total !== 0 && response.recordset[0].total > 0) ? true : false;
}
...
async function isDiplomaExist(data) {
  ...
  let parameters = [
    ...
    { name: "SchoolListID", dataType: "Int", value: data.school_id },
    { name: "CollegeDegreeID", dataType: "Int", value: data.degree_id },
    { name: "CollegeCourseID", dataType: "Int", value: data.course_id }
  ];

  if (data.major_id !== null) parameters.push({ name: "CollegeMajorID", dataType: "Int", value: data.major_id });
  ...
  parameters = [
    { name: "EmployeeID", dataType: "VarChar", value: data.employee_id },
    { name: "Diploma", dataType: "VarChar", value: data.diploma }
  ];
...
async function getMajors(courseID) {
  ...
  const parameters = [
    { name: "CourseID", dataType: "Int", value: courseID }
  ];
