...
async function hasChange(data) {
  ...
  let parameters = [
    ...
    { name: "LastName", dataType: "VarChar", value: data.last_name },
    { name: "FirstName", dataType: "VarChar", value: data.first_name },
    { name: "MiddleName", dataType: "VarChar", value: data.middle_name },
    {
      name: "MiddleInitial",
      dataType: "VarChar",
      value: data.middle_name.charAt(0),
    },
    { name: "ExtName", dataType: "VarChar", value: data.extension_name },
    { name: "Nickname", dataType: "VarChar", value: data.nick_name },
    { name: "Email", dataType: "VarChar", value: data.personal_email },
    { name: "MobileNo", dataType: "VarChar", value: data.mobile_no },
    { name: "PermanentTelNo", dataType: "VarChar", value: data.telephone_no },
    { name: "CivilStatus", dataType: "Int", value: data.civil_status_id },
    { name: "ReligionCode", dataType: "Int", value: data.religion_id },
    {
      name: "ContactPerson",
      dataType: "VarChar",
  ...
  parameters = [{ name: "EmployeeID", dataType: "VarChar", value: data.employee_id }];

  if (data.current_address.room_or_floor_or_unit_no_and_building_name.trim() !== "")
  {
    parameters.push({ name: "CurrentAddress_RoomOrFloorOrUnitNoAndBuildingName", dataType: "VarChar", value: data.current_address.room_or_floor_or_unit_no_and_building_name });
  }

  if (data.current_address.house_or_lot_and_block_no.trim() !== "")
  {
    parameters.push({ name: "CurrentAddress_HouseOrLotAndBlockNo", dataType: "VarChar", value: data.current_address.house_or_lot_and_block_no });
  }

  if (data.current_address.street_name.trim() !== "")
  {
    parameters.push({ name: "CurrentAddress_StreetName", dataType: "VarChar", value: data.current_address.street_name });
  }

  if (data.current_address.sub_division.trim() !== "")
  {
    parameters.push({ name: "CurrentAddress_Subdivision", dataType: "VarChar", value: data.current_address.sub_division });
  }

  if (data.current_address.region_code.trim() !== "")
  {
    parameters.push({ name: "CurrentAddress_RegionCode", dataType: "VarChar", value: data.current_address.region_code });
  }

  if (data.current_address.province_code.trim() !== "")
  {
    parameters.push({ name: "CurrentAddress_ProvinceCode", dataType: "VarChar", value: data.current_address.province_code });
  }
 
  if (data.current_address.city_or_municipality_code.trim() !== "")
  {
    parameters.push({ name: "CurrentAddress_CityOrMunicipalityCode", dataType: "VarChar", value: data.current_address.city_or_municipality_code });
  }

  if (data.current_address.barangay.trim() !== "")
  {
    parameters.push({ name: "CurrentAddress_Barangay", dataType: "VarChar", value: data.current_address.barangay });
  }

  if (data.permanent_address.room_or_floor_or_unit_no_and_building_name.trim() !== "")
  {
    parameters.push({ name: "PermanentAddress_RoomOrFloorOrUnitNoAndBuildingName", dataType: "VarChar", value: data.permanent_address.room_or_floor_or_unit_no_and_building_name });
  }
  
  if (data.permanent_address.house_or_lot_and_block_no.trim() !== "")
  {
    parameters.push({ name: "PermanentAddress_HouseOrLotAndBlockNo", dataType: "VarChar", value: data.permanent_address.house_or_lot_and_block_no });
  }
  
  if (data.permanent_address.street_name.trim() !== "")
  {
    parameters.push({ name: "PermanentAddress_StreetName", dataType: "VarChar", value: data.permanent_address.street_name });
  }
  
  if (data.permanent_address.sub_division.trim() !== "")
  {
    parameters.push({ name: "PermanentAddress_Subdivision", dataType: "VarChar", value: data.permanent_address.sub_division });
  }
  
  if (data.permanent_address.region_code.trim() !== "")
  {
    parameters.push({ name: "PermanentAddress_RegionCode", dataType: "VarChar", value: data.permanent_address.region_code });
  }
  
  if (data.permanent_address.province_code.trim() !== "")
  {
    parameters.push({ name: "PermanentAddress_ProvinceCode", dataType: "VarChar", value: data.permanent_address.province_code });
  }
   
  if (data.permanent_address.city_or_municipality_code.trim() !== "")
  {
    parameters.push({ name: "PermanentAddress_CityOrMunicipalityCode", dataType: "VarChar", value: data.permanent_address.city_or_municipality_code });
  }
  
  if (data.permanent_address.barangay.trim() !== "")
  {
    parameters.push({ name: "PermanentAddress_Barangay", dataType: "VarChar", value: data.permanent_address.barangay });
  }
...
function toActualColumnName(columnName) {
  ...
  } else if (columnName === "relationship_id") {
    return "ContactFamilyTypeID";
  } else if (columnName === "room_or_floor_or_unit_no_and_building_name") {
    return "RoomOrFloorOrUnitNoAndBuildingName";
  } else if (columnName === "house_or_lot_and_block_no") {
    return "HouseOrLotAndBlockNo";
  } else if (columnName === "street_name") {
    return "StreetName";
  } else if (columnName === "sub_division") {
    return "Subdivision";
  } else if (columnName === "region_code") {
    return "RegionCode";
  } else if (columnName === "province_code") {
    return "ProvinceCode";
  } else if (columnName === "city_or_municipality_code") {
    return "CityOrMunicipalityCode";
  } else if (columnName === "barangay") {
    return "Barangay";
  } else if (columnName === "is_same_as_current_address") {
    return "IsSameAsCurrentAddress";
  } else {
    throw `Cannot find the Actual Column name of ${columnName}`;
  }
}
...
async function getAllProvinces(regionCode) {
  const query = `SELECT 
               ...
               ORDER BY [NAME] ASC`;
  const parameters = [
    { name: "RegionCode", dataType: "VarChar", value: regionCode },
  ];
...
async function getAllCitiesOrMunicipalities(provinceCode) {
  const query = `SELECT 
               ...
               ORDER BY [NAME] ASC`;
  const parameters = [
    { name: "ProvinceCode", dataType: "VarChar", value: provinceCode },
  ];
  return (await helperMethods.executeQuery(query, parameters)).recordset;
}
