...
const getUserLeaveBalanceDetails = async (sqlWhereStrArr, args) => {
  // try {
  const leaveBalanceQuery = await sqlHelper.query(
    `
        SELECT
          e.Class,
          e.EmployeeCode,
  ...
  // Format the Remaining value
  const formattedResult = leaveBalanceQuery.map((item) => {
...
const getForfeitedLeave = async (sqlWhereStrArr, args) => {
  return await sqlHelper.query(
    `
    SELECT *
    FROM
...
//   }
// },
const checkEmployeeToApprove = async (sqlWhereStrArr, args) => {
  return await sqlHelper.query(
    `SELECT
        code,
        lvl,
...
const verifyLevelCreateLeave = async (employeeId) => {
...
const getAttributedYear = async (
  ...
  const leaveBalances = await sqlHelper.query(
    `DECLARE @appliedLeave MONEY = ?
      ...
      (
        SELECT RowNum, YearEffectivity, YearAttributed, RemainingLeave, AppliedLeave,
            Balance = AppliedLeave - SUM(RemainingLeave) OVER (PARTITION BY YearEffectivity ORDER BY RowNum)
        FROM @TestTable
      )
      UPDATE t SET
          DistrbutedLeave = CASE
          WHEN Balance > 0 THEN bal.RemainingLeave
          WHEN Balance < 0 AND bal.RemainingLeave + Balance >= 0 THEN bal.RemainingLeave + Balance
          WHEN Balance = 0 AND bal.RemainingLeave = bal.AppliedLeave THEN bal.AppliedLeave
          ELSE 0
          END
      FROM cteRunningBalance bal INNER JOIN @TestTable t ON bal.RowNum = t.RowNum;
    
      SELECT YearAttributed AS year, DistrbutedLeave AS daysOfLeave
      FROM @TestTable
      WHERE DistrbutedLeave IS NOT NULL
      AND DistrbutedLeave != 0;`,
...
const checkDateOfLeaveOverlap = async (employeeID, dateFrom, dateTo) => {
  return await sqlHelper.query(
    `SELECT *
        FROM
            [UE database]..VacationSickLeave
        WHERE
            ...
            AND IDCode = ?
            AND (status NOT IN ('CANCELLED', 'RejectedByLevel1', 'RejectedByLevel2') OR cancelledStatusOrig = 'Approved')
        ORDER BY RecNo DESC
        `,
    [dateFrom, dateTo, dateFrom, dateTo, employeeID],
  );
};
