...
async function getAllFields(moduleID) {
  ...
  const parameters = [
    { name: "PISModuleNameID", dataType: "Int", value: Number(moduleID) }
  ];
...
async function getList(fieldName, statusID, description) {
 ...
 if (fieldName === "COURSES") {
  ...
  const parameters = [
    { name: "Description", dataType: "VarChar", value: `%${  description  }%` }
  ];
...
async function submit(body) {
  ...
  const parameters = [
    { name: "Description", dataType: "VarChar", value: body.description },
    { name: "IsActive", dataType: "Bit", value: body.is_active },
    { name: "EmployeeID", dataType: "VarChar", value: body.employee_id },
  ];
  ...
  if (body.action_name === "ADD NEW") {
      ...
      query = `UPDATE HR..${body.table_name} 
        ...
        if (body.action_name === "EDIT") parameters.push({ name: "ID", dataType: "Int", value: body.id });
  }

  if (body.table_name === "CollegeMajors") parameters.push({ name: "CollegeCourseID", dataType: "Int", value: body.course_id });
