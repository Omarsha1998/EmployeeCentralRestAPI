...
async function getCivilStatusName(civilStatusID) {
  const query = `SELECT TOP 1 
                ...
                WHERE ID = @ID`;
  const parameters = [{ name: "ID", dataType: "Int", value: Number(civilStatusID) }];
...
async function getReligionName(religionCode) {
  ...
  const parameters = [{ name: "ReligionCode", dataType: "VarChar", value: religionCode }];
  const response = await helperMethods.executeQuery(query, parameters);
  return response.recordset[0].Name;
}
...
async function getRelationshipName(relationshipID) {
  ...
  const parameters = [{ name: "ID", dataType: "Int", value: Number(relationshipID) }];
  const response = await helperMethods.executeQuery(query, parameters);
  return response.recordset[0].Name;
}
...
async function getInstitutionName(schoolID) {
  ...
  const parameters = [{ name: "ID", dataType: "Int", value: Number(schoolID) }];
  const response = await helperMethods.executeQuery(query, parameters);
  return response.recordset[0].Name.toUpperCase();
}

function getParameterID(id){
 return [{ name: "ID", dataType: "Int", value: Number(id) }];
}
...
async function getRegionName(regionCode) {
  const query = `SELECT TOP 1 
                ...
                WHERE CODE = @CODE`;
  const parameters = [{ name: "CODE", dataType: "VarChar", value: regionCode }];
  const response = await helperMethods.executeQuery(query, parameters);
  return response.recordset[0].Name;
}
...
async function getProvinceName(provinceCode) {
  const query = `SELECT TOP 1 
                ...
                WHERE CODE = @CODE`;
  const parameters = [{ name: "CODE", dataType: "VarChar", value: provinceCode }];
  const response = await helperMethods.executeQuery(query, parameters);
  return response.recordset[0].Name;
}
...
async function getCityOrMunicipalityName(cityOrMunicipalityCode) {
  const query = `SELECT TOP 1 
                ...
                WHERE CODE = @CODE`;
  const parameters = [{ name: "CODE", dataType: "VarChar", value: cityOrMunicipalityCode }];
  const response = await helperMethods.executeQuery(query, parameters);
  return response.recordset[0].Name;
}
...
async function getRequestedFields(requestID, statusID) {
  ...
  const parameters = [{ name: "ID", dataType: "Int", value: requestID }];
  const response = await helperMethods.executeQuery(query, parameters);
  ...
  if (requestedFields.includes("(SIBLINGS)") === true || requestedFields.includes("(CHILDREN)") === true) {
    ...
    const parameters = [
      ...
      { name: "CurrentStatus", dataType: "Int", value: statusID }
    ];
...
async function getDescription(fullName, requestID) {
  ...
  let parameters = [{ name: "ID", dataType: "Int", value: requestID }];
  ...
  switch (destinationTable) {
    ...
    case "Family":
      if (familyType === 0) {
        ...
        if (familyType === 1) {
          // ------------------------------------------------- EDIT -------------------------------------------------
          description += "mother";
          description += await getFullName("Mother", createdBy);
          // ------------------------------------------------- EDIT -------------------------------------------------
        } else if (familyType === 2) {
          // ------------------------------------------------- EDIT -------------------------------------------------
          description += "father";
          description += await getFullName("Father", createdBy);
          // ------------------------------------------------- EDIT -------------------------------------------------
          ...
          // ------------------------------------------------- EDIT -------------------------------------------------
          if (requestType === 0) {
            description += await getFullName("Spouse", createdBy);
          }
          // ------------------------------------------------- EDIT -------------------------------------------------
          // ------------------------------------------------- CREATE -------------------------------------------------
          else if (requestType === 1) {
    ...
    case "License":
      ...
      parameters = [
        { name: "EmployeeID", dataType: "VarChar", value: createdBy },
        { name: "ID", dataType: "Int", value: requestID }
      ];
      ...
      parameters = [
        { name: "EmployeeID", dataType: "VarChar", value: createdBy },
        { name: "LicenseNo", dataType: "VarChar", value: response.recordset[0].license_no }
      ];
...
async function getFamilyID(requestID) {
  ...
  const parameters = [{ name: "RequestHdrID", dataType: "Int", value: requestID }];
...
async function isMoreThanOne(requestID, isEditRequestType = true) {
  ...
  const parameters = [{ name: "RequestHdrID", dataType: "Int", value: requestID }];
  const response = await helperMethods.executeQuery(query, parameters);
  return response.recordset.length > 1 ? true : false;
}
...
async function getFullName(famType, createdBy, familyID = 0) {
  ...
  const parameters = [{ name: "EmployeeID", dataType: "VarChar", value: createdBy }];

  if (familyID !== 0 && familyID > 0) parameters.push({ name: "Recno", dataType: "Int", value: familyID });
...
async function getPending(employeeID, dateFrom, dateTo) {
  ...
  for (const index in requestHdr) {
    ...
    const parameters = [{ name: "RequestHdrID", dataType: "Int", value: requestHdr[index].request_id }];
...
async function approveRequest(employeeID, data) {
  ...
  try {
    ...
    for (const index in data) {
      ...
      let parameters = [{ name: "ID", dataType: "Int", value: requestDtlID }];
      ...
      parameters = [
        { name: "ApprovedBy", dataType: "VarChar", value: employeeID },
        { name: "ID", dataType: "Int", value: requestDtlID },
      ];
      ...
      if (data[index].column_name !== undefined && data[index].value !== undefined) {
        ...
        if (columnName === "DEGREE" || columnName === "COURSE") {
           ...
           query = `SELECT TOP 1
                   ...
                   parameters = [{ name: "Description", dataType: "VarChar", value: degreeOrCourseName }];
                   response = await helperMethods.executeQuery(query, parameters, transaction);     
                   ...
                   if (!isExist){
                    ...
                    parameters = [
                      { name: "Description", dataType: "VarChar", value: degreeOrCourseName },
                      { name: "IsActive", dataType: "Bit", value: true },
                      { name: "CreatedBy", dataType: "VarChar", value: employeeID },
                    ];
                  }
          ...
          query = `SELECT TOP 1
                   ...
                   FROM HR..CollegeCourses 
                   WHERE [Description] = @Description`;
                   parameters = [{ name: "Description", dataType: "VarChar", value: data[index].course_id }];
                   response = await helperMethods.executeQuery(query, parameters, transaction);     
          ...
          query = `SELECT TOP 1
                   ...
                   FROM HR..${tableName} 
                   WHERE [Description] = @Description`;
          parameters = [{ name: "Description", dataType: "VarChar", value: majorName }];
          response = await helperMethods.executeQuery(query, parameters, transaction);     
          if (response.recordset[0].total > 0) isExist = true; 
          ...
          if (!isExist){
            ...
            parameters = [
              { name: "IsActive", dataType: "Bit", value: true },
              { name: "CollegeCourseID", dataType: "Int", value: courseID },
              { name: "Description", dataType: "VarChar", value: majorName },
              { name: "CreatedBy", dataType: "VarChar", value: employeeID },
            ];
          }
        }
        ...
        query = `SELECT TOP 1 
                 ...
                 WHERE [Description] = @Description 
                 ORDER BY ID DESC`;
        parameters = [{ name: "Description", dataType: "VarChar", value: value}];
        response = await helperMethods.executeQuery(query, parameters, transaction);
        ...
        query = `UPDATE [UE database]..RequestDtl 
                 ...
                 WHERE ID = @ID`;
        parameters = [
          { name: "NewValue", dataType: "VarChar", value: latestID },
          { name: "ID", dataType: "Int", value: requestDtlID },
        ];
        response = await helperMethods.executeQuery(query, parameters, transaction);
        if (response.rowsAffected[0] === 0) throw "No rows affected";
      ...
      parameters = [{ name: "ID", dataType: "Int", value: requestDtlID }];
      ...
      if (requestType === "Edit") {
        ...
        if (familyType === "None" && licenseNo === "") {
          ...
          if (columnName.includes("CurrentAddress_") === true || columnName.includes("PermanentAddress_") === true) {
            ...
            parameters = [{ name: "EmployeeCode", dataType: "VarChar", value: createdBy }];
          ...
          parameters = [
            { name: "NewValue", dataType: "VarChar", value: newValue.toString() },
        ...
        // ------------------------------------------------- FAMILY BACKGROUNDS -------------------------------------------------
          ...
          if (columnName === "MARRIAGE CERTIFICATE" || columnName === "BIRTH CERTIFICATE") {
            if (columnName === "MARRIAGE CERTIFICATE") {
              ...
              await manipulateFolder(transaction, requestHdrID, "BIRTH CERTIFICATE", "approved", createdBy, requestDtlID);
            }
            ...
            parameters = [
              { name: "UpdatedBy", dataType: "VarChar", value: createdBy },
              { name: "DateTimeUpdated", dataType: "DateTime", value: dateTimeCreated },
              { name: "EmployeeCode", dataType: "VarChar", value: createdBy },
              { name: "FamType", dataType: "VarChar", value: familyType },
              { name: "Recno", dataType: "Int", value: familyID },
            ];
            response = await helperMethods.executeQuery(query, parameters, transaction);
            if (response.rowsAffected[0] === 0) throw "No rows affected";
          }
      ...
      // ------------------------------------------------- CREATE -------------------------------------------------
        ...
        if (familyType !== "None" && licenseNo === "") {
          ...
          parameters = [{ name: "RequestHdrID", dataType: "Int", value: requestHdrID }];
          response = await helperMethods.executeQuery(query, parameters, transaction);
    ...
    if (familyDetails.length > 0) {
      ...
      for (const obj of familyDetails) {
       ...
       (SELECT NewValue FROM [UE database]..RequestDtl WHERE ColumnName = 'CompanySchool' AND RequestHdrID = @RequestHdrID AND PersonQueueNumber = @PersonQueueNumber) AS 'CompanySchool'`;
        ...
        let parameters = [
          { name: "RequestHdrID", dataType: "Int", value: (obj.RequestHdrID === undefined) ? obj[0].RequestHdrID : obj.RequestHdrID },
          { name: "PersonQueueNumber", dataType: "SmallInt", value: (obj.PersonQueueNumber === undefined) ? obj[0].PersonQueueNumber : obj.PersonQueueNumber },
        ];
        ...
        parameters = [
          { name: "EmployeeCode", dataType: "VarChar", value: result.EmployeeCode },
          { name: "FullName", dataType: "VarChar", value: result.FullName },
          { name: "Birthdate", dataType: "Date", value: result.Birthdate },
          { name: "Occupation", dataType: "VarChar", value: result.Occupation },
          { name: "MarriageDate", dataType: "Date", value: result.MarriageDate },
          { name: "FamType", dataType: "VarChar", value: result.FamType },
          { name: "CompanySchool", dataType: "VarChar", value: result.CompanySchool },
          { name: "HasAttachment", dataType: "Bit", value: hasAttachment }
        ];
    ...
    if (educationalBackgroundsRequestHdrID !== 0) {
     let query = `SELECT 
      ...
      let parameters = [{ name: "RequestHdrID", dataType: "Int", value: educationalBackgroundsRequestHdrID }];
      ...
      parameters = [
        { name: "EmployeeCode", dataType: "VarChar", value: result.employee_code },
        { name: "From", dataType: "VarChar", value: result.from },
        { name: "To", dataType: "VarChar", value: result.to },
        { name: "SchoolListID", dataType: "Int", value: result.school_list_id },
        { name: "CollegeDegreeID", dataType: "Int", value: result.college_degree_id },
        { name: "CollegeCourseID", dataType: "Int", value: result.college_course_id },
      ];

      if (result.college_major_id !== undefined && result.college_major_id !== null) parameters.push({ name: "CollegeMajorID", dataType: "Int", value: result.college_major_id });
      response = await helperMethods.executeQuery(query, parameters, transaction);
      if (response.rowsAffected[0] === 0) throw "No rows affected";
    }
    // ------------------------------------------------- EDUCATIONAL BACKGROUNDS -------------------------------------------------
    ...
    if (trainingOrSeminarRequestHdrID !== 0) {
      ...
      let parameters = [{ name: "RequestHdrID", dataType: "Int", value: trainingOrSeminarRequestHdrID }];
      let response = await helperMethods.executeQuery(query, parameters, transaction);
      const result = response.recordset[0];
      ...
      parameters = [
        { name: "EmployeeCode", dataType: "VarChar", value: result.employee_code },
        { name: "TrainingOrSeminarName", dataType: "VarChar", value: result.training_or_seminar_name },
        { name: "TrainingProvider", dataType: "VarChar", value: result.training_provider },
        { name: "FromDate", dataType: "VarChar", value: result.from_date },
        { name: "ToDate", dataType: "VarChar", value: result.to_date },
        { name: "Venue", dataType: "VarChar", value: result.venue },
        { name: "CreatedBy", dataType: "VarChar", value: result.employee_code }
      ];
  ...
  try {
    ...
    for (const index in data.ids) {
      if (requestHDRID === 0) {
        ...
        let parameters = [{ name: "ID", dataType: "Int", value: data.ids[index].id }];
        let response = await helperMethods.executeQuery(query, parameters, transaction);
        ...
        parameters = [{ name: "ID", dataType: "Int", value: requestHDRID }];
        response = await helperMethods.executeQuery(query, parameters, transaction);
        if (response.rowsAffected[0] === 0) throw "No rows affected";
      }
      ...
      const parameters = [
        { name: "RemarksBy", dataType: "VarChar", value: employeeID },
        { name: "HRRemarks", dataType: "VarChar", value: data.hr_remarks },
        { name: "ID", dataType: "Int", value: data.ids[index].id },
      ];
...
async function manipulateFolder(transaction, requestID, type, statusFolder, employeeID = 0, requestDtlID = 0) {
  ...
  let parameters = [
    ...
    { name: "Type", dataType: "VarChar", value: type }
  ];

  if (requestDtlID !== 0) {
    query += " AND D.ID = @ID";
    parameters.push({ name: "ID", dataType: "Int", value: requestDtlID });
  }
  ...
  if (isFound === true) {
    ...
    if (statusFolder === "approved" && employeeID > 0) {
      ...
      if (type === "MARRIAGE CERTIFICATE") path += "/family_backgrounds/spouse";
        ...
        parameters = [{ name: "RequestHdrID", dataType: "Int", value: requestID }];
        response = await helperMethods.executeQuery(query, parameters, transaction);
