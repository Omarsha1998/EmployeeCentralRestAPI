...
function getURL() {
  const port =
    process.env.DEV === "true" ? process.env.PORT_DEV : process.env.PORT;
  return `${process.env.EC_PROTOCOL + process.env.EC_IP}:${port}`;
}
...
const sqlConnection = sqlHelper.returnSQL();
...
async function executeQuery(query, parameters = null, transaction = null) {
  ...
  if (parameters !== null) {
    parameters.forEach((obj) => {
      obj.dataType = getActualDataType(obj.dataType);
...
function getActualDataType(parameter) {
  if (parameter.toLowerCase() === "varchar") return mssql.VarChar;
  else if (parameter.toLowerCase() === "bit") return mssql.Bit;
  else if (parameter.toLowerCase() === "int") return mssql.Int;
  else if (parameter.toLowerCase() === "smallint") return mssql.SmallInt;
  else if (parameter.toLowerCase() === "date") return mssql.Date;
  else if (parameter.toLowerCase() === "datetime") return mssql.DateTime;
